...
const delay = (ns) => new Promise ((resolve) => { setTimeout (resolve, ms) });
...
export default async function handler(req, res) {
   ...
   async function putMethod () {
    try {
        ...
        await delay (1000);
        const speaker = JSON.parse(readFileData).speakers;
        if (!speaker) {
            res.status(404).send("Error: Request faild with status code 404");
        } else {
...
async function postMethod () {
    try {
        ...
        if (!speaker) {
            ...
            writeFile(
                jsonFile,JSON.stringify({speakers: newSpeakersArray}, null, 2))
            res.setHeader("Content-Type", "application/json");
            res.status (200).send(JSON.stringify(recordFromBody,null,2));
            console.log("PUT /api/speakers status: 200"); 
        }
...
async function deletetMethod () {
    try {
        ...
        if (!speaker) {
            ...
            writeFile(jsonFile,JSON.stringify(speakers.find(rec=> rec.id == id),null, 2))
            res.setHeader("Content-Type", "application/json");
            res.status (200).send(JSON.stringify(recordFromBody,null,2));
